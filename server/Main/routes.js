...
router.posts('/api/posts/commenttodb',(req, res, next) =>{
  const values=[req.body.comment, req.body.user_id, req.body.usernmae, req.body.post_id]
...
router.put('/api/put/commenttodb', (req, res, next) =>{
  const values=[req.body.comment, req.body.user_id, req.body.post_id , req.body.usernmae,req.body.cid]
...
router.post('/api/post/userprofiletodb',(req, res, next) =>{
  ...
  pool.query(`INSERT INTO users(username, email , email_verified, date_created) VALUES($1,$2,$3, NOW()) ON CONFLICT DO NOTHING`, values, (q_err, q_res) =>{
    res.json(q_res.rows)
    console.log(q_err)
